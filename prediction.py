...
year_df.loc[:, "total_touchdowns"] = year_df["reception_td"] + year_df["run_td"]
year_df.loc[:, "total_yards"] = year_df["receiving_yards"] + year_df["rushing_yards"]

# Calculate averages for each season
for season in seasons:
    # Filter the DataFrame for the specific season
    season_data = year_df[year_df["season"] == season].copy()
...
players_2023_df = week_df[week_df["season"] == 2023].copy()
...
players_2023_df.loc[:, "total_touchdowns"] = (
        players_2023_df["reception_td"] + players_2023_df["run_td"]
)
players_2023_df.loc[:, "total_yards"] = (
        players_2023_df["receiving_yards"] + players_2023_df["rushing_yards"]
)
...
average_stats = (
    week_df.groupby(["player_name", "season"])
    .agg(
...
players_df["Avg Receptions per Season"] = (
        players_df["Career Receptions (2023)"] / players_df["Seasons Played"]
)
players_df["Avg TDs per Season"] = (
        players_df["Career Total TDs (2023)"] / players_df["Seasons Played"]
)
players_df["Avg Games per Season"] = (
        players_df["Career Games (2023)"] / players_df["Seasons Played"]
)
...
normalized_df = year_df.groupby("player_name", group_keys=False).apply(
    normalize_seasons
)
...
def create_wr_baseline_comparison(normalized_df):
    ...
    wr_df = normalized_df[
        normalized_df["position"].str.upper() == "WR"
        ].copy()  # Ensure 'WR' matches correctly

    # Calculate total_touchdowns and total_yards for baseline
    wr_df.loc[:, "total_touchdowns"] = wr_df["reception_td"] + wr_df["run_td"]
    wr_df.loc[:, "total_yards"] = wr_df["receiving_yards"] + wr_df["rushing_yards"]
    wr_df.loc[:, "total_performance"] = (
    ...
    baseline_df = (
        wr_df.groupby("t")
        .agg(total_performance=("total_performance", "mean"))
        .reset_index()
    )
    ...
    selected_df = normalized_df[
        normalized_df["player_name"].str.lower().isin(selected_players)
    ].copy()

    # Calculate total performance for selected players
    selected_df.loc[:, "total_touchdowns"] = (
            selected_df["reception_td"] + selected_df["run_td"]
    )
    selected_df.loc[:, "total_yards"] = (
            selected_df["receiving_yards"] + selected_df["rushing_yards"]
    )
    selected_df.loc[:, "total_performance"] = (
            selected_df["total_touchdowns"]
            + selected_df["total_yards"]
            + selected_df["receptions"]
    )

    # Merge selected players with the baseline data on 't'
    comparison_df = selected_df.merge(
        baseline_df, on="t", suffixes=("_player", "_baseline")
    )
    ...
    for player_name in selected_players:
        player_data = comparison_df[
            comparison_df["player_name"].str.lower() == player_name
            ]
        fig.add_trace(
            go.Scatter(
                x=player_data["t"],
