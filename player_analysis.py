...
def calculate_average_performance(df, season_range):
    ...
    avg_touchdowns = (
        filtered_data.groupby("season")["total_touchdowns"]
        .mean()
        .reindex(seasons, fill_value=0)
        .round(2)
    )
    avg_yards = (
        filtered_data.groupby("season")["total_yards"]
        .mean()
        .reindex(seasons, fill_value=0)
        .round(2)
    )
    avg_receptions = (
        filtered_data.groupby("season")["receptions"]
        .mean()
        .reindex(seasons, fill_value=0)
        .round(2)
    )
...
def generate_average_graphs(season_range):
    ...
    avg_touchdowns, avg_yards, avg_receptions = calculate_average_performance(
        year_df, season_range
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=seasons, y=avg_touchdowns, mode="lines+markers", name="Avg Touchdowns"
        ),
        row=1,
        col=1,
    )
    fig.add_trace(
        go.Scatter(x=seasons, y=avg_yards, mode="lines+markers", name="Avg Yards"),
        row=1,
        col=2,
    )
    fig.add_trace(
        go.Scatter(
            x=seasons, y=avg_receptions, mode="lines+markers", name="Avg Receptions"
        ),
        row=1,
        col=3,
    )
...
def generate_bar_graph(data, title, x_label, y_label):
    ...
    colors = sample_colorscale(
        "Pinkyl", [i / (len(players) - 1) for i in range(len(players))]
    )

    fig = go.Figure(
        data=[go.Bar(x=players, y=values, marker=dict(color=colors), name=title)]
    )
    fig.update_layout(
...
def update_player_dropdowns(season_range):
    filtered_df = year_df[
        year_df["season"].between(season_range[0], season_range[1])
    ].copy()
    player_names = filtered_df["player_name"].dropna().unique()  # Drop NaN values
    dropdown_options = [
        {"label": player, "value": player} for player in player_names if player
...
def update_graph(season_range, player1, player2, player3):
    ...
    filtered_df["total_touchdowns"] = (
        filtered_df["reception_td"] + filtered_df["run_td"].copy()
    )
    filtered_df["total_yards"] = (
        filtered_df["receiving_yards"] + filtered_df["rushing_yards"].copy()
    )

    average_stats = (
        filtered_df.groupby("player_name")
        .agg({"total_touchdowns": "mean", "receptions": "mean", "total_yards": "mean"})
        .rename(
            columns={
...
def calculate_total_touchdowns(df, season_range):
    df = df[df["season"].between(season_range[0], season_range[1])].copy()
    df["total_touchdowns"] = df["reception_td"] + df["run_td"].copy()
    return (
        df.groupby("player_name")["total_touchdowns"]
        .sum()
...
def plot_seaborn_weighted_scores(players, rf_predictions, seasons_played):
    ...
    plt.bar(
        ...
        color=sns.color_palette("Greens", len(players)),
        alpha=0.8,
    )
                    ...
                    [
                        html.Div(
                            [
                                html.Label("Select Player 1:"),
                                dcc.Dropdown(
                                    id="player1-1-dropdown", style={"width": "100%"}
                                ),
                            ],
                            style={
                        ...
                        html.Div(
                            [
                                html.Label("Select Player 2:"),
                                dcc.Dropdown(
                                    id="player2-1-dropdown", style={"width": "100%"}
                                ),
                            ],
                            style={
                                "display": "inline-block",
                        ...
                        html.Div(
                            [
                                html.Label("Select Player 3:"),
                                dcc.Dropdown(
                                    id="player3-1-dropdown", style={"width": "100%"}
                                ),
                            ],
                            style={
                                "display": "inline-block",
...
def update_dropdown_and_graph(selected_players):
    ...
    baseline_graph = create_wr_baseline_comparison(
        normalized_df, selected_players, player_colors
    )
...
def update_weighted_graph(player1, player2, player3):
    ...
    metrics["seasons_played"] = (
        year_df.groupby("player_name")["season"]
        .nunique()
        .reindex(metrics["player_name"])
        .fillna(1)
    )
